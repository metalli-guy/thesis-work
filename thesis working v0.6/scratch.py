...
def connect():
    ...
    try:
        ...
        b3.config(state=tkinter.DISABLED, bg="green")
        b4.config(state=tkinter.NORMAL, bg=defaultbg, fg="black")
        time.sleep(0.5)
...
def disconnect():
    ...
    try:
        if ser is not None and ser.is_open:
            ...
            ser.reset_output_buffer()
            ser.close()
            b3.config(state=tkinter.NORMAL, bg=defaultbg, fg="black")
            b4.config(state=tkinter.DISABLED, bg="red")
    except NameError:
...
def poweron():
    try:
        ...
        b6.config(state=tkinter.DISABLED, bg="green")
...
def poweroff():
    try:
        ...
        ser.reset_output_buffer()
        ser.write(selection.encode('utf-8'))
        b7.config(state=tkinter.DISABLED, bg="red")
...
dataframe.grid(row=2, column=2, columnspan=2, padx=10, pady=10, sticky="nsew")
...
l2 = Label(connection_frame, text=arduino_ports[0], font=("Arial", 12, "bold"))
...
guidelines = [
    ...
    'Click "Connect" in the Connections Frame to connect to Arduino.',
