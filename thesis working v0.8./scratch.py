...
def disconnect():
    ...
    try:
        if ser is not None and ser.is_open:
            ...
            IRpower.config(text="Power", fg="black", font=("Arial", 12, "bold"))
...
def latch():
    try:
        if illuminate != 0:
            selectionbin = (1 / seconds) * 1000000
            selection = 'L ' + str(selectionbin) + '\n\r'
            ser.reset_input_buffer()
            ser.reset_output_buffer()
            ser.write(selection.encode('utf-8'))
            print(selection)
...
def resetting():
    try:
        IRpower.config(text="Power", fg="black", font=("Arial", 12, "bold"))
        BLpower.config(text="Power", fg="black", font=("Arial", 12, "bold"))
        pmtpower.config(text="PMT Power", fg="black", font=("Arial", 12, "bold"))
        b7.config(state=tkinter.NORMAL)
        b6.config(state=tkinter.NORMAL)
        bl8.config(state=tkinter.NORMAL)
